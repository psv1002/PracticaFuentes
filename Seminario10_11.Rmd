...
union <-full_join(x = medicamentosAntidepresivos, y = muerteDemencia, by = c("Comunidades")) %>%
  ...
  summarise(sexo = c("Hombres", "Mujeres", "Total"),
            Demencia = c(sum(HombreDemencia), sum(MujerDemencia), sum(TotalDemencia)),
            Antidepresivos = c(sum(HombreAntidepresivos), sum(MujerAntidepresivos), sum(TotalAntidepresivos)))%>%
  pivot_longer(names_to = "condicion", values_to = "valores", cols = (Demencia:Antidepresivos))

ggplot(union, aes(fill = condicion, y=valores, x=Sexo)) + 
...
Codigo nuevo grafico
```{r}
unionDF<- data.frame( 
  Sexo = c("Mujer","Hombre"),
  MuertesDemencia = c(sum(union$MujerDemencia), sum(union$HombreDemencia)),
  ConsumoAntidepresivos = c(sum(union$MujerAntidepresivos), sum(union$HombreAntidepresivos))
)
unionDF<-unionDF %>% 
  pivot_longer(names_to ="condicion", values_to = "valores", cols = c(MuertesDemencia:ConsumoAntidepresivos))

ggplot(unionDF, aes(fill = condicion, y=valores, x=Sexo)) + 
    geom_bar(position="dodge", stat="identity")
```
