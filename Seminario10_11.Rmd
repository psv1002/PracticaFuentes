...
```{r eval=FALSE}
#view((enfermedades2))
MuertesAlceimer<-enfermedades2 %>% 
  rename(Comunidades = `Defunciones según causa de muerte. Avance enero-mayo 2020`, Defunciones = ...2)
  slice(.data =. ,28:31)
view(MuertesAlceimer)
...
enf<-enfermedades2 %>% 
  rename(Comunidades = `Defunciones según causa de muerte. Avance enero-mayo 2020`, Defunciones = ...2) %>% 
  slice(.data = ., 28:31) %>% 
  select("Comunidades", "Defunciones")
view(enf)
```
...
```{r}
union <-left_join(x = MedicamentosAntidepresivos, y = enf, by = c("Comunidades"))
union
```
