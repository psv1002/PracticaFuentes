...
muerteIsquemia <- data.frame(
  Comunidades = comunidadesI,
  Total = as.integer(totalI),
  Hombre = as.integer(hombreI),
  Mujer = as.integer(mujerI))
muerteIsquemia
```
...
medicamentosAntidepresivos<- data.frame(
  ...
  select(.data = medicamentos2,...39)%>%slice(9:28)%>%rename(Total =...39),
  select(.data = medicamentos2,...39)%>%slice(31:50)%>%rename(Hombre =...39),
  select(.data = medicamentos2,...39)%>%slice(52:71)%>%rename(Mujer =...39)
)
...
MedicamentosAntidepresivos <- medicamentos2%>%
  ...
  drop_na() %>% 
  mutate(.data = ., AntidepresivosSI = as.double(AntidepresivosSI))
...
union <-full_join(x = MedicamentosAntidepresivos, y = muerteDemencia, by = c("Comunidades")) %>%
  arrange(.data = ., Total, AntidepresivosSI)
```
...
```{r eval=FALSE}
grafico_MuertesDemencia <- ggplot(data = union, aes(x = AntidepresivosSI, y = Total)) +
