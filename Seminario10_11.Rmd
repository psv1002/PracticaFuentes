...
```{r eval=FALSE}
# Creamos una tabla llamada Enfe igual que la tabla inicial Enfermedades2 pero a partir de la fila 7 para quedarnos con los datos que nos interesan
Enfe <- enfermedades2%>%
...
unionDem <-full_join(x = medicamentosAntidepresivos, y = muerteDemencia, by = c("Comunidades")) %>%
  ...
  summarise(Sexo = c("Hombres", "Mujeres", "Total"),
            Demencia = c(sum(HombreDemencia,na.rm = TRUE)*100/poblacionEspaña2020, sum(MujerDemencia,na.rm = TRUE)*100/poblacionEspaña2020, sum(TotalDemencia,na.rm = TRUE)*100/poblacionEspaña2020),
...
#Este codigo de la gráfica ya esta en el apartado de las gráficas
...
unionPan <-full_join(x = medicamentosAntidepresivos, y = muerteCancerPancreas, by = c("Comunidades")) %>%
  arrange(.data = ., TotalCancerPancreas, TotalAntidepresivos)%>%
  summarise(Sexo = c("Hombres", "Mujeres", "Total"),
            CancerPancreas = c(sum(HombreCancerPancreas, na.rm = TRUE)*100/poblacionEspaña2020, sum(MujerCancerPancreas, na.rm = TRUE)*100/poblacionEspaña2020, sum(TotalCancerPancreas, na.rm = TRUE)*100/poblacionEspaña2020),
            Antidepresivos = c(mean(HombreAntidepresivos), mean(MujerAntidepresivos), mean(TotalAntidepresivos)))%>%
...
unionIsq <-full_join(x = medicamentosAntidepresivos, y = muerteIsquemia, by = c("Comunidades")) %>%
  arrange(.data = ., TotalIsquemia, TotalAntidepresivos)%>%
  summarise(Sexo = c("Hombres", "Mujeres", "Total"),
            Isquemia = c(sum(HombreIsquemia, na.rm = TRUE)*100/poblacionEspaña2020, sum(MujerIsquemia, na.rm = TRUE)*100/poblacionEspaña2020, sum(TotalIsquemia, na.rm = TRUE)*100/poblacionEspaña2020),
            Antidepresivos = c(mean(HombreAntidepresivos), mean(MujerAntidepresivos), mean(TotalAntidepresivos)))%>%
...
graficaIsq+facet_grid("Variables", scales = "free_y")
```
Prueba de gráfica como dijo en Tutoría solo de comunidades y antidepresivos/estimulantes
```{r}
comunidadesAntidepresivos <- pivot_longer(data = medicamentosAntidepresivos, names_to = "Variables", values_to = "Porcentajes", cols = (TotalAntidepresivos:MujerAntidepresivos))

graficaAntidepresivos<-ggplot(comunidadesAntidepresivos, aes(fill = Variables, y=Comunidades, x=Porcentajes)) + 
    geom_bar(position="dodge", stat="identity")
graficaAntidepresivos+facet_grid("Variables", scales = "free_y")
```
